...
region_data['자녀 인구수'] = region_data[children_ages].replace(",", "", regex=True).astype(int).sum(axis=1)
region_data['학부모 인구수'] = region_data[parents_ages].replace(",", "", regex=True).astype(int).sum(axis=1)
...
for region in regions:
    ...
    ax.plot(['자녀 인구수', '학부모 인구수'], [region_subset['자녀 인구수'].values[0], region_subset['학부모 인구수'].values[0]], label=region)
